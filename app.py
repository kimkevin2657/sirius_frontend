...
def add_user():
    if request.method == 'POST':
        ...
        conn.commit()
        cur.execute("INSERT INTO usersetting (id, username, margin, minordcost, maxordcost, minordrate, maxordrate, trailingstop, leverage, scriptnumber, currentposition, enteringtrade, bot, positionamount, stoploss, stopgain, stoplossbot, stopgainbot, trailingstopbot) VALUES (%s, %s, %s ,%s ,%s ,%s ,%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", (newid, request.form['username'], float(request.form['margin'])/100, float(request.form['minordcost']), float(request.form['maxordcost']), float(request.form['minordrate'])/100, float(request.form['maxordrate'])/100, float(request.form['trailingstop']), float(request.form['leverage']), int(request.form['scriptnumber']), 'exit',False,True,0.0, float(request.form['stoploss'])/100, float(request.form['stopgain'])/100, True, True, True))
        conn.commit()
